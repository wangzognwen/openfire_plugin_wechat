...
public class JerseyWrapper extends ServletContainer {
	...
	private static final String AUTHFILTER = AuthFilter.class.getName();
	
	/** The Constant CORSFILTER. */
	private static final String CORSFILTER = CORSFilter.class.getName();
	...
	static {
		...
		System.out.println(SCAN_PACKAGE_DEFAULT);
		prc.setPropertiesAndFeatures(config);	
		try {
			Class clazz = Class.forName(CORSFILTER);
			prc.getProperties().put(CONTAINER_RESPONSE_FILTERS, clazz);
	...
	public static String loadAuthenticationFilter() {
		...
		try {
			clazz = Class.forName(AUTHFILTER);
		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}
		
		prc.getProperties().put(CONTAINER_REQUEST_FILTERS, clazz);	
		return loadingStatusMessage;
	}
